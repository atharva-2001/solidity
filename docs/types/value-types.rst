...
introduced type and ``V`` has to be a built-in value type (the "underlying type"). The type ``C`` does not have any
operators or bound member functions. In particular, even the operator ``==`` is not defined.
...
For now, inline assembly is required to define useful operations on such types.
...
.. code-block:: solidity
    ...
    /// Unwrapping function, returns the internal representation.
    /// Should only be used internally.
    function unwrap(UFixed256x18 x) pure returns (uint256 r) {
        assembly { r := x }
    }
    /// Wrapping function, turns an internal representation into the
    /// user defined type. Should only be used internally.
    function wrap(uint256 x) pure returns (UFixed256x18 r) {
        assembly { r := x }
    }
    ...
    library FixedMath {
        ...
        function truncate(UFixed256x18 a) internal pure returns (uint256) {
            return unwrap(a) / multiplier;
        }
        /// Turns a uint256 into a UFixed256x18 of the same value.
        /// Reverts if the integer is too large.
        function toUFixed256x18(uint256 a) internal pure returns (UFixed256x18) {
            return wrap(a * multiplier);
        }
    }

Note how ``wrap`` and ``FixedMath.toUFixed256x18`` have the same signature
but perform two very different operations: The ``wrap`` function
is an internal function that returns a ``UFixed256x18`` that has the
same data representation as the input, while ``toUFixed256x18`` returns
